...
const createPlayground = asyncHandler(async (req, res) => {
  ...
  fs.mkdir(
    path.resolve(process.env.PLAYGROUNDS_PATH, `${createdCollaboration._id}`),
    {
      recursive: true,
    }
  );
...
const getFiles = asyncHandler(async (req, res) => {
  const { playgroundId } = req.params;
  const folderPath = path.resolve(process.env.PLAYGROUNDS_PATH, playgroundId);
